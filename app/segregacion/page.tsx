...
const bebidas: MaterialInfo[] = [
  { id: "pet-1", title: "Botellas de Agua/Gaseosa", image: "./Segregacion/Botellas.png", description: "Plástico PET altamente reciclable. Debe entregarse limpio y compactado.", steps: ["Enjuagar bien", "Secar o escurrir", "Aplastar verticalmente", "Tapa por separado"], destiny: "TACHO BLANCO", colorClass: "bg-white text-slate-900", advice: "La compactación vertical maximiza la eficiencia del almacenamiento en la base." },
  { id: "hdpe-2", title: "Yogurt, Lácteos y Shampoo", image: "./Segregacion/Lacteos.png", description: "Plásticos densos que requieren eliminación total de residuos orgánicos.", steps: ["Enjuague obligatorio", "Quitar etiquetas", "Aplastar", "Sin tapa"], destiny: "TACHO BLANCO", colorClass: "bg-white text-slate-900", advice: "Lave inmediatamente después del uso para evitar fermentación y olores." },
  ...
  { id: "tetra", title: "Envases Tetra Pak", image: "./Segregacion/Tetra_pak.png", description: "Multicapa (cartón, aluminio, plástico). Se gestiona junto al papel y cartón.", steps: ["Enjuagar bien", "Desplegar esquinas", "Aplastar", "Secar"], destiny: "TACHO AMARILLO", colorClass: "bg-yellow-400 text-black", advice: "Desplegar las pestañas permite un aplastado total, ahorrando 70% de espacio." },
  { id: "vidrio", title: "Botellas de Vidrio", image: "./Segregacion/Vidrio.png", description: "Envases de salsas o bebidas. 100% reciclable infinitas veces.", steps: ["Eliminar líquidos", "Enjuague obligatorio", "Quitar tapa o corcho", "Depositar sin romper"], destiny: "TACHO VERDE", colorClass: "bg-emerald-500 text-white", advice: "Evite romper el vidrio; los fragmentos pequeños son difíciles de segregar." }
];
...
const ciencia: MaterialInfo[] = [
  ...
  // NUEVO 2
  { id: "guantes-epp", title: "Guantes y EPP", image: "./Segregacion/Guantes.png", description: "Guantes de nitrilo o látex usados en experimentos. Contienen trazas de reactivos.", steps: ["Retirar sin tocar exterior", "Doblar y envolver", "Colocar en bolsa roja", "Sellar y rotular"], destiny: "TACHO ROJO", colorClass: "bg-red-600 text-white", advice: "EPP contaminados pueden contener trazas de reactivos o microorganismos que comprometen la bioseguridad." },
  // NUEVO 3
...
const mantenimiento: MaterialInfo[] = [
  ...
  // NUEVO 5
  { id: "aceites", title: "Aceites y Lubricantes", image: "./Segregacion/Aceites.png", description: "Residuos de generadores eléctricos. Hidrocarburos tóxicos.", steps: ["Envase hermético", "Rotular 'Aceite usado'", "Evitar mezcla con agua", "Retorno obligatorio"], destiny: "TACHO ROJO", colorClass: "bg-red-600 text-white", advice: "Su disposición está estrictamente prohibida en campo; retorno obligatorio." },
  // NUEVO 6
  { id: "trapos", title: "Trapos contaminados", image: "./Segregacion/Trapos.png", description: "Textiles con grasa, combustible o solventes. Inflamables.", steps: ["Bolsa resistente", "No exprimir", "Mantener cerrado", "Entregar a gestión"], destiny: "TACHO ROJO", colorClass: "bg-red-600 text-white", advice: "Deben segregarse en contenedores metálicos con tapa hermética." },
  // NUEVO 7
  { id: "chatarra", title: "Piezas Metálicas / Chatarra", image: "./Segregacion/Chatarra.png", description: "Pernos, cables, repuestos oxidados. Componentes metálicos.", steps: ["Separar piezas limpias", "Agrupar por metal", "Sin bordes sueltos", "Depositar en azul"], destiny: "TACHO AZUL", colorClass: "bg-blue-600 text-white", advice: "Priorice la valorización mediante reciclaje trasladándolo hacia un operador autorizado." }
];

const comida: MaterialInfo[] = [
  { id: "vegetal-1", title: "Frutas y Verduras", image: "./Segregacion/Organicos.png", description: "Residuos biodegradables. Pueden atraer fauna si no se manejan bien.", steps: ["Separar cáscaras y semillas", "Evitar exceso de humedad", "Depositar sin bolsas", "Tapar contenedor"], destiny: "TACHO MARRÓN", colorClass: "bg-amber-800 text-white", advice: "Su manejo evita la atracción de fauna silvestre hacia la base científica." },
  // NUEVO 8
  ...
  // NUEVO 9
  { id: "borra", title: "Borra de Café y Té", image: "./Segregacion/Borra.png", description: "Filtros de café y saquitos de té usados (sin líquidos).", steps: ["Retirar líquido", "Sin plásticos", "Bolsa cerrada"], destiny: "TACHO MARRÓN", colorClass: "bg-amber-800 text-white", advice: "Son sustratos orgánicos de alta degradabilidad." }
];

const higiene: MaterialInfo[] = [
  { id: "sanitarios-1", title: "Residuos de Baño", image: "./Segregacion/Bano.png", description: "Papel higiénico y toallitas. Carga biológica no aprovechable.", steps: ["Depósito directo", "No mezclar con reciclables", "Bolsa resistente", "Sellar al llenar"], destiny: "TACHO NEGRO", colorClass: "bg-slate-700 text-white", advice: "Pequeñas cantidades pueden inutilizar materiales reciclables por contaminación." },
  // NUEVO 10
  { id: "mascarillas", title: "Mascarillas y Limpieza", image: "./Segregacion/Mascarillas.png", description: "Trapos domésticos y mascarillas descartables. Riesgo sanitario.", steps: ["Doblar o envolver", "Introducir en bolsa cerrada", "Depositar en negro"], destiny: "TACHO NEGRO", colorClass: "bg-slate-700 text-white", advice: "El embolsado inmediato reduce riesgos de exposición." },
  // NUEVO 11
